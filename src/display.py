...
def display3DCuts(brain_data, seg_data=None, brain_color_map='bone', seg_color_map='jet', seg_alpha=0.5, window_title='Brain MRI - 3D Visualization'):
	"""
	...
	:param string window_title: title for the figure window
	:return int: -1 for error
	"""
...
def displayPred3DCuts2(brain_data, seg_data, pred_data, image_color_map='bone', seg_color_map='jet', seg_alpha=0.5, window_title='Ground Truth vs Prediction Comparison', show_seg_in_top=True):
	...
	:param string seg_alpha: segmentation alpha (defines how much segmentation will be transparent: 1 being oppac)
	:param string window_title: title for the figure window
	:param bool show_seg_in_top: if True, show segmentation overlay in top row (ground truth), otherwise show plain brain
	:return int: -1 for error
	"""
	...
	# Determine if we're showing ground truth or just original brain
	has_segmentation = np.any(seg_data > 0)
	top_row_label = "GROUND TRUTH" if has_segmentation else "ORIGINAL BRAIN"
	...
	# Set title for each ax
	ax[0][0].set_title(f"{top_row_label} - Sagittal {half_X}/{brain_data.shape[0] - 1}", fontsize=10, fontweight='bold')
	ax[0][1].set_title(f"{top_row_label} - Coronal {half_Y}/{brain_data.shape[1] - 1}", fontsize=10, fontweight='bold')
	ax[0][2].set_title(f"{top_row_label} - Horizontal {half_Z}/{brain_data.shape[2] - 1}", fontsize=10, fontweight='bold')
	ax[1][0].set_title(f"PREDICTION - Sagittal {half_X}/{brain_data.shape[0] - 1}", fontsize=10, fontweight='bold', color='darkred')
	ax[1][1].set_title(f"PREDICTION - Coronal {half_Y}/{brain_data.shape[1] - 1}", fontsize=10, fontweight='bold', color='darkred')
	ax[1][2].set_title(f"PREDICTION - Horizontal {half_Z}/{brain_data.shape[2] - 1}", fontsize=10, fontweight='bold', color='darkred')
	...
	def update_X(val):
		...
		ax[0][0].set_title(f"{top_row_label} - Sagittal {int(layer)}/{brain_data.shape[0] - 1}", fontsize=10, fontweight='bold')
		ax[1][0].set_title(f"PREDICTION - Sagittal {int(layer)}/{brain_data.shape[0] - 1}", fontsize=10, fontweight='bold', color='darkred')
		fig.canvas.draw_idle()
	...
	def update_Y(val):
		...
		ax[0][1].set_title(f"{top_row_label} - Coronal {int(layer)}/{brain_data.shape[1] - 1}", fontsize=10, fontweight='bold')
		ax[1][1].set_title(f"PREDICTION - Coronal {int(layer)}/{brain_data.shape[1] - 1}", fontsize=10, fontweight='bold', color='darkred')
		fig.canvas.draw_idle()
	...
	def update_Z(val):
		...
		ax[0][2].set_title(f"{top_row_label} - Horizontal {int(layer)}/{brain_data.shape[2] - 1}", fontsize=10, fontweight='bold')
		ax[1][2].set_title(f"PREDICTION - Horizontal {int(layer)}/{brain_data.shape[2] - 1}", fontsize=10, fontweight='bold', color='darkred')
		fig.canvas.draw_idle()
	...
	# Only overlay segmentation in top row if we have actual ground truth data
	if has_segmentation:
